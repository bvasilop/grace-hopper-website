...
$(document)
  .ready(function () {
    ...
    // login css pre click added css styling for buttons. adding margins to fix
    // original off-center buttons
    $('.btn-sm').css({'margin': '5px 0 5px 5px', 'font-size': '17px'});

    // added css styling post click to adjust off-center button at top of page
    $('.form-control').css({'margin': '5px 0 5px 5px', 'font-size': '17px'});

    // set an event handler for clicking the 'login' button use '.btn-sm' class to
    // listen for click
    $('.btn-sm').click(function () {

      // when button is clicked, the '#login-form' field is hidden
      $('.btn-sm').css({'margin-bottom': '5px', 'font-size': '17px'});
      $('#login-form').hide();

      // when button is clicked, show '.user-fullname' fade in at 650ms
      $('.user-info').fadeIn(650);

      // use css to add padding to top and increase font size
      $('.user-info').css({'padding-top': '5px', 'font-size': '20px'});
    ...
    $('.view-details').on('click', function (event) {
      ...
      //  create variable 'container' in order to tell function where the location of
      // the target is in relation to targetElement
      var container = targetElement.parentElement.parentElement;

      //  finding 'details' class within container for each element and alter
      // visibility
      $(container)
        .find('.details')
        .each(function (index, el) {
          ...
          if ($(el).is(':visible')) {
            ...
            // alter text within button with 'view details' because the 'details' have been
            // hidden by the "fadeOut('fast')"
            targetElement.innerText = "View Details"
            // allows element to 'fadeIn' when not visible
          } else {
            $(el).fadeIn(650);
            // alter text within buttons with 'Hide Details' since details are visible with
            // "fadeIn('fast')" user is able to 'Hide Details'
    ...
    $('button.vote').on('click', function (event) {
      // create variable for 'userVote' assign variable 'userVote' to 'data-vote'
      // attribute
      var userVote = $(this).data('vote');
      // set counter to increment by 1 each time button is clicked for 'great'
      if (userVote == 'great') {
        voteCounts.great++;
        voteCounts.total // set counter to increment by 1 each time button is clicked for 'greatest'++;
